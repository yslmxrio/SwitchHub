...
def main(serial_com_port):
    try:
        ser = serial.Serial(serial_com_port, BAUD_RATE, timeout=1)
    except serial.SerialException as e:
        print(f"Error: Could not open port {serial_com_port}.")
        print(f"Details: {e}")
        print("Please check your port, permissions, and ensure no other program is using it.")
        return

    print(f"Successfully connected to {serial_com_port}. Waiting for switch output...")
    print("="*60)
    ...
    try:
        while True:
            ...
            # Handle case where switch skips dialog and goes straight to prompt
            elif state == "BOOTING" and "switch>".lower() in buffer.lower():
                print("\n[SCRIPT] >>> Switch booted directly to prompt. Skipping dialog... <<<")
                IS_BOOTING = False
                # We are already at Switch>, so go to the state that handles 'enable'
                send_command(ser, "enable")
                buffer = ""
                # This state expects to see 'switch#' next, which is correct
                state = "NEGATE_INIT_CONFIG_DIALOG_DONE"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Error: No COM Port provided.")
        sys.exit(1)
    serial_com_port_arg = sys.argv[1]
    main(serial_com_port_arg)